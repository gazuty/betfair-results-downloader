...
from .csv_utils import clean_and_remove_duplicates
...
def run_pipeline(
    *,
    ...
    plan = {
        ...
        "last_settled_date_utc": str(last_settled_date_utc)
        if last_settled_date_utc
        else None,
    ...
    unique_markets_enriched = (
        unique_markets_after if unique_markets_after else unique_markets_input
    )
    ...
    if cache_hits > 0:
        ...
        unique_markets_from_cache = max(
            min(
                unique_markets_enriched - unique_markets_from_api,
                unique_markets_enriched,
            ),
            0,
        )
    ...
    try:
        audit_summary = compute_missing_settled_dates(
            csvr.canonical_path, window_days=90
        )
    except Exception as e:
        audit_summary = {"message": f"Audit failed: {e}"}
    ...
    try:
        ...
        run_state = {
            ...
            "last_requested_from_utc": dl.from_utc,
            "last_requested_to_utc": dl.to_utc,
            "rows_fetched": dl.rows_downloaded,
            "rows_after_dedupe": rows_after_dedupe,
            "rows_written": csvr.rows_in_canonical,
            "canonical_path": str(csvr.canonical_path),
            "snapshot_path": str(csvr.snapshot_path),
        }
        state_path = results_dir / "run_state.json"
        history_path = results_dir / "run_history.jsonl"
        say(f"STATE: writing run_state to {state_path}")
        save_run_state(state_path, run_state)
        append_run_history(history_path, run_state)
        persist_success = True
    except Exception as e:
        persist_error = str(e)
        say(f"WARNING: failed to persist run state: {e}")
    ...
    if not config.enable_azure_sql:
        ...
        say(
            "Publishing to Azure SQL..."
            if (not config.dry_run)
            else "Dry-run: Azure publish will be skipped."
        )
